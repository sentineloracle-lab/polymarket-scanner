...
def get_ai_client():
    ...
    # Utilisation de l'URL v1beta sans le suffixe /openai/ pour tester la compatibilité directe
    return OpenAI(
        api_key=gemini_key,
...
def analyze_market_with_ai(client, model_name, market_data):
    ...
    try:
        # Note : On utilise 'models/gemini-1.5-flash' car c'est le standard Google
...
def run_aggressive_scanner(markets, prompts_dir):
    ...
    logging.info(f"Démarrage du scan avec Gemini 1.5 Flash")
    ...
    for market in markets:
        ...
        if ai_res:
            ...
            append_to_csv([time.strftime("%Y-%m-%d %H:%M:%S"), market['question'], market['id'], market['volume'], market['liquidity'], "ERROR_API", "N/A", 0, 0, "Échec appel Gemini"])
